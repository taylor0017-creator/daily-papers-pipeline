...
description: |
  ...
  支持两种模式：
  - 精读模式（触发："精读 1,3,5"）：只深读用户指定索引的论文
  - 完整模式（触发："批量笔记"、"跑一下论文笔记"）：深读分流表中所有"必读"论文
---
...
解析 args 参数确定当前运行模式，记录 `TARGET_INDICES` 变量供后续使用：

- **精读模式**：args 包含逗号分隔的数字（如 `"1,3,5"`）
  → `TARGET_INDICES = [1, 3, 5]`，只深读推荐文件中对应索引的论文
- **完整模式**：args 为空或包含 "batch"/"all"
  → `TARGET_INDICES = None`，深读分流表中所有"必读"论文
...
**精读模式**（用户指定了索引）：
1. 读取今天的推荐文件，扫描 `### N.` 开头的论文段落
2. 只匹配 `TARGET_INDICES` 中的索引（例如用户说"精读 1,3"，只匹配 `### 1.` 和 `### 3.`）
3. 从匹配的段落中提取论文标题和 arXiv 链接
4. 对每篇入选论文，使用 Task agent 调用 `/paper-reader` skill（传入 arXiv 链接）
5. paper-reader 会自动补充概念库，无需重复
