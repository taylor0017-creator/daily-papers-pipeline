...
**主题分组规则**：优先使用论文的 `cluster_scores` 字段进行数据驱动的分类。`cluster_scores` 是一个 dict，key 为话题簇 ID，value 为命中分数。每篇论文属于得分最高的簇（primary cluster）。也支持交叉簇论文（多个 cluster 都有分数）。在确保有 `cluster_scores` 的情况下应以簇分组为骨架；没有时再按传统方式手动分类。簇 ID 映射：A1_World_Model→World Model, A2_Robot_Manipulation→Robot Manipulation, A3_Embodied_Navigation→Embodied Navigation, B1_GPU_Training→GPU Training, B2_GPU_Inference→GPU Inference, C1_Diffusion_Gen→Diffusion & Generative Models。
...
- 💡 **想精读？** 回复 `精读 N`    ← 仅对"值得看"等级的论文显示，"必读"不显示（精读由你选择），"可跳过"不需要
```
...
date: {date}
arxiv_id: {arxiv_id}
tags: [light-note, auto-generated]
---

# {title}

**作者**: {authors}
**机构**: {affiliations or "未知"}
**日期**: {date}
**来源**: {source} {upvotes_text}
**链接**: [arXiv]({url}) | [PDF]({pdf})

## 方法概要

{method_summary 或 从 section_headers + abstract 提取的要点}
...
**写入方式**：用 Write 工具逐篇写入。确保 `{NOTES_PATH}/_轻笔记/` 目录存在。

**跳过规则**：
- 如果 `{NOTES_PATH}/_轻笔记/{filename}.md` 已存在，**覆盖更新**（数据比旧版本新）
- 不需要质量检查（轻笔记本身内容简单）

## 输出

完成后告知用户：
- 推荐了多少篇论文，必读/值得看/可跳过各多少篇
- 所有论文已生成轻笔记到 `_轻笔记/` 目录
- 提示：**想精读哪篇？回复「精读 1,3,5」指定序号**，会自动生成完整深笔记
